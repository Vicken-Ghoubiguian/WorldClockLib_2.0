...
char* sprintfWorldClock_2_0(struct worldClock_2_0 worldClock_2_0ToDisplay)
{
	...
	//
	sprintf(resultString, "\033[0;32m{timezone : %s | date : %s/%s/%s | time : %s:%s:%s | number of the day in the week : %s | number of the week in the year : %s | number of the day in the year : %s}\033[0m", s_timezone, s_dmonth, s_month, s_year, s_hour, s_minute, s_second, s_dweek, s_nweek, s_dyear);
...
char* sprintfLike_time_t(struct like_time_t time_tToDisplay)
{
	...
	//
	sprintf(resultString, "\033[0;32m{timezone : %s | datetime : %ld | number of the week in the year : %s}\033[0m", s_timezone, time_tToDisplay.dt_as_time_t, s_nweek);
